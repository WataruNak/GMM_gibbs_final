...
def custom_export(players):
    yield [
        "session", "participant_code", "started_time", "c_ari", "d_count",
    ...
    for p in players:
        ...
        session = p.session
        if "d_count" in p.participant.vars:
            yield [
                session.code, participant.code, participant.time_started_utc, p.c_ari, participant.d_count,
...
class Categorize(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        player.participant.d_count += player.count
